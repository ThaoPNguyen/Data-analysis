...
RLeg = 0.32062 + 0.98421*LLeg + 0.8344652 (Sigma = 0.8344652 is the error or variance of the model)

LLeg = (RLeg - 0.32062 - 0.8344652)/0.98421
...
change_muscle_day <- change_muscle_day %>% 
	mutate(staygroup = case_when(Stayicu <= 10 ~ "10 days",
															 Stayicu > 10 & Stayicu <= 15 ~ "11-15 days",
															 Stayicu > 15 & Stayicu <= 20 ~ "16-20 days",
															 Stayicu > 20 & Stayicu <= 25 ~ "21-25 days",
															 Stayicu > 25 & Stayicu <= 30 ~ "26-30 days",
															 TRUE ~ "More than 30 days"))

change_muscle_day %>% ungroup() %>% select(stage1:staygroup) %>% tbl_summary(by = staygroup, missing = "no")

```
Muscle stage2 show larger change in days 21-25.
...
One metaanalysis called "The rate and assessment of muscle wasting during critical illness: a systematic review and meta-analysis" concluded that ICU patient will loss 2% of skeletal muscle per day during the first week of ICU admission, so that should be a strong argument for the generalized muscle wasting in Tetanus and CNS.
