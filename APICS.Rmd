...
subdata <- admission %>% 
	select(Ablett, CAUTIclinical, CAUTI_CDC, DailyAssessment.urinary_catheterization) %>% 
	filter(DailyAssessment.urinary_catheterization == "Yes")

tbl_summary(subdata, by = Ablett, statistic = list(all_continuous() ~ "{median} ({p25}, {p75})", all_categorical() ~ "{n} / {N} ({p}%)")) %>% add_p()

subdata_count <- admission %>% 
	select(patient_id, Ablett, DailyAssessment.urinary_catheterization, CAUTI_CDC) %>% 
	filter(DailyAssessment.urinary_catheterization == "Yes")
...
tb1 <- table(subdata_count$Ablett, subdata_count$CAUTI_CDC) %>% 
	as.data.frame() %>% 
	filter(Var2 == "Yes")

tb <- left_join(tb, tb1, by = c("Ablett" = "Var1"))

tb <- tb %>% mutate(divide = (Freq/sum)*1000)

tb %>% select(Ablett, divide) %>% tbl_summary(by = "Ablett", type = list(divide ~ 'continuous'), statistic = list(all_continuous() ~ "{mean}")) %>% add_p()
##Re-make

vent <- admission %>% 
	select(patient_id, Ablett, Timeonset, Ventday)
...
vent %>% select(Ablett, Timeonset) %>% tbl_summary(by = "Ablett") %>% add_p()

mecha <- vent %>% filter(Ventday != 0) %>% select(Ablett, Ventday) %>% tbl_summary(by = "Ablett") %>% add_p()
...
ablett <- admission %>% 
	select(patient_id, Ablett) %>% 
	left_join(retain1, by = "patient_id") %>% 
	filter(!is.na(countday)) %>% 
	mutate(length = as.numeric(length),
				 length = case_when(length <= 5 ~ "1-5 Days",
				 									 length >5 & length < 10 ~ "6-10 Days",
				 									 TRUE ~ "> 10 Days"))

ablett %>% select(Ablett, length) %>% tbl_summary(by = "Ablett") %>% add_p()

ablett_group <- ablett %>% left_join(cdcCAUTI, by = "patient_id") %>% mutate(CAUTI_CDC = ifelse(is.na(CAUTI_CDC), "No", CAUTI_CDC))

ablett_group %>% tbl_summary(include = c(length, CAUTI_CDC), by = "length") %>% add_p()

##Group cạth flow chart

group <- admission %>% select(patient_id, Urinecath, CAUTI_CDC)
...
frequen_anti <- frequen_anti %>% 
	group_by(patient_id, DailyAssessment.date_of_daily_assessment) %>% 
	mutate(listanti = list(Antibiotics)) %>% 
	select(-c(DailyAssessment.urinary_catheterization, clinical)) %>% 
	unique() %>% 
	ungroup()

frequen_anti$DailyAssessment.date_of_daily_assessment <- as.Date(frequen_anti$DailyAssessment.date_of_daily_assessment)

frequen_anti <- frequen_anti %>% 
	group_by(patient_id) %>% 
	mutate(countday = cumsum(c(1, diff(DailyAssessment.date_of_daily_assessment) > 1)))

##

anti <- left_join(clinicalCAUTI, frequen_anti, by = c("patient_id"))

library(janitor)

anti_use <- tabyl(anti$Antibiotics, sort = TRUE) %>% 
	arrange(desc(n)) %>% 
...
kbl(anti_use, format = "html", table.attr = "style='width:50%;'") %>% kable_styling(bootstrap_options = c("striped"))
##Sites separated

sitepatient <- select(admission, patient_id, Site)
